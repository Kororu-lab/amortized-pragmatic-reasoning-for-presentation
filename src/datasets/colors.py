"""
Colors in Context dataset loader and preprocessing.
"""
import os
import math
import pickle
...
from constants import SOS_TOKEN, EOS_TOKEN, PAD_TOKEN, UNK_TOKEN
from logger import get_logger

logger = get_logger(__name__)

# Auto-download punkt tokenizer if not present
try:
...
class ColorsInContext(data.Dataset):
    ...
    def __init__(
        ...
        logger.debug('Initializing ColorsInContext dataset')
        ...
        if not os.path.isfile(cache_clean_data):
            ...
            logger.info(f'Saving cleaned data to {cache_clean_data}')
            with open(cache_clean_data, 'wb') as fp:
                pickle.dump(data, fp)
        else:
            logger.info(f'Loading cleaned data from {cache_clean_data}')
        ...
        if vocab is None:
            ...
            logger.debug('Using provided vocabulary')
            self.vocab = vocab
