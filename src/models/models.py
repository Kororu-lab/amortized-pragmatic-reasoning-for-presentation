"""
Neural network models for the amortized RSA project.
Includes Speaker, Listener, and LanguageModel implementations.
"""
...
from constants import PAD_IDX, SOS_IDX, EOS_IDX
...
class FeatureMLP(nn.Module):
    """
    Simple MLP for feature transformation.
    """
    def __init__(self, input_size: int = 16, output_size: int = 16):
...
def to_onehot(y: torch.Tensor, n: int = 3) -> torch.Tensor:
    """
    ...
    """
    y_onehot = torch.zeros(y.shape[0], n, device=y.device)
...
class Speaker(nn.Module):
    """
    Pragmatic speaker model that generates referring expressions.
    
    The speaker takes visual features and a target object index, then generates
    a sequence of tokens to refer to the target object.
    """
    def __init__(self, feat_model: nn.Module, embedding_module: nn.Embedding, 
                 hidden_size: int = 100):
        """
        Initialize the speaker model.
        
        Args:
            feat_model: Vision encoder model
            embedding_module: Word embedding module
    ...
    def forward(
        ...
        """
        Generate language from image features.
        
        Args:
            feats: Visual features (batch_size, n_obj, ...)
            targets: Target object indices (batch_size,)
            greedy: Whether to use greedy decoding
            activation: Activation type ('gumbel', 'softmax', 'multinomial', etc.)
            tau: Temperature for softmax/Gumbel
            length_penalty: Whether to apply length penalty
            max_len: Maximum sequence length
            
        Returns:
            Tuple of (language tensor, lengths, eos_loss, language probabilities)
        """
        ...
        for i in range(max_len - 2):  # Have room for SOS, EOS if never sampled
            ...
            if greedy:
                ...
                if length_penalty:
                    ...
                    eos_prob.append(idx_prob[:,EOS_IDX])
            ...
            for j, pred in enumerate(predicted_npy):
                if not done_sampling[j]:
                    lang_length[j] += 1
                if pred == EOS_IDX and activation in {'gumbel', 'multinomial'}:
        ...
        if activation == 'multinomial':
            ...
            outputs = self.outputs2vocab(outputs)       # outputs: (B,V)
            idx_prob = F.log_softmax(outputs, dim=1)
            lang_prob.append(idx_prob[:, EOS_IDX].unsqueeze(1))
    ...
    def to_text(self, lang_onehot, vocab):
        """
        Convert one-hot encoded language to text.
        
        Args:
            lang_onehot: One-hot encoded language tensor
            vocab: Vocabulary dictionary with 'i2w' mapping
...
class LiteralSpeaker(nn.Module):
    """
    Literal speaker model that learns to generate language from ground truth examples.
    """
    def __init__(self, feat_model: nn.Module, embedding_module: nn.Embedding, 
                 hidden_size: int = 100, contextual: bool = True):
        """
        Initialize the literal speaker model.
        
        Args:
            feat_model: Vision encoder model
            embedding_module: Word embedding module
            hidden_size: Hidden size for GRU
            contextual: Whether to use contextual features (all objects) or just target
        """
    ...
    def forward(
        ...
        """
        Forward pass for literal speaker.
        
        Args:
            feats: Visual features
            seq: Input sequence (one-hot or indices)
            length: Sequence lengths
            y: Target object indices
    ...
    def sample(self, feats: torch.Tensor, y: torch.Tensor, 
               greedy: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Generate from image features using greedy search or sampling.
        ...
        Returns:
            Tuple of (sampled sequences, sequence lengths)
        """
        with torch.no_grad():
            batch_size = feats.shape[0]
            if self.contextual:
